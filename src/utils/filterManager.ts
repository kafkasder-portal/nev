...
export interface FilterValidationRule {
  ...
  rules: {
    ...
    max?: number
    pattern?: string
    custom?: (value: any, allFilters: FilterState) => string | null
  }
}
...
export class FilterManager {
  ...
  getFieldsToClear(changedField: string, newValue: any): string[] {
  ...
  buildQueryParams(filters: FilterState): Record<string, any> {
  ...
  private evaluateCondition(value: any, condition: FilterDependency['condition']): boolean {
  ...
  static buildFilterQuery(filters: FilterState): Record<string, any> {
  ...
  static applyDependencies(
    ...
    return fields.map(field => {
      ...
      for (const dependency of fieldDependencies) {
        ...
        const conditionMet = this.evaluateCondition(sourceValue, dependency.condition)
  ...
  private static evaluateCondition(value: any, condition: FilterDependency['condition']): boolean {
    const { operator, value: conditionValue } = condition
