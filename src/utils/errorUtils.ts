...
export const parseApiError = (error: any): Omit<AppError, 'id' | 'timestamp'> => {
  if (error.response) {
...
// Helper functions
const getZodErrorMessage = (issue: any): string => {
  switch (issue.code) {
    case 'invalid_type':
      return `Geçersiz veri türü. ${issue.expected} bekleniyor, ${issue.received} alındı.`;
    case 'invalid_string':
    ...
    case 'too_small':
      if (issue.type === 'string') {
        return `En az ${issue.minimum} karakter olmalıdır.`;
      }
      if (issue.type === 'number') {
        return `En az ${issue.minimum} olmalıdır.`;
      }
      if (issue.type === 'array') {
        return `En az ${issue.minimum} öğe seçmelisiniz.`;
      }
    ...
    case 'too_big':
      ...
      if (issue.type === 'array') {
        return `En fazla ${issue.maximum} öğe seçebilirsiniz.`;
      }
      return 'Değer çok büyük.';
    case 'invalid_enum_value':
      return `Geçerli değerler: ${issue.options.join(', ')}`;
    case 'invalid_date':
