 ...
 */
export const exportBeneficiariesToExcel = (beneficiaries: any[], filters?: any) => {
 ...
 */
export const exportApplicationsToExcel = (applications: any[], filters?: any) => {
  const columns: ExportColumn[] = [
    { key: 'id', label: 'ID', width: 10 },
    { key: 'applicant_name', label: 'Başvuran', width: 25 },
    { key: 'type', label: 'Başvuru Türü', width: 20 },
    { key: 'status', label: 'Durum', width: 15 },
    { key: 'amount', label: 'Tutar', width: 15, formatter: (value) => value ? `${value} TL` : '' },
 ...
 */
export const exportDonationsToExcel = (donations: any[], filters?: any) => {
  const columns: ExportColumn[] = [
    { key: 'id', label: 'ID', width: 10 },
    { key: 'donor_name', label: 'Bağışçı', width: 25 },
    { key: 'amount', label: 'Tutar', width: 15, formatter: (value) => `${value} TL` },
...
export const exportReportToExcel = (reportData: {
  ...
  if (reportData.donations && reportData.donations.length > 0) {
    ...
    const donationsData = reportData.donations.map(d => [
      ...
      d.donation_date ? new Date(d.donation_date).toLocaleDateString('tr-TR') : ''
    ])
