...
export function RealtimeActivityFeed({ 
  ...
  const subscribeToRealTimeUpdates = () => {
    ...
    const unsubscribeTasks = realtimeService.subscribe('tasks', (event) => {
      let activityTitle = ''
      let activityDescription = ''
      let severity: 'info' | 'success' | 'warning' | 'error' = 'info'

      const newRecord = event.new as any
      const oldRecord = event.old as any
      ...
      if (activityTitle && newRecord) {
        addActivity({
          ...
          user_id: newRecord.assigned_to || newRecord.assigned_by,
          created_at: new Date().toISOString(),
          category: 'task',
          severity,
    ...
    const unsubscribeMeetings = realtimeService.subscribe('meetings', (event) => {
      if (event.eventType === 'INSERT') {
        ...
        if (newRecord) {
          addActivity({
            ...
            user_id: newRecord.organizer_id,
            created_at: new Date().toISOString(),
            category: 'meeting',
            severity: 'info',
    ...
    const unsubscribeMessages = realtimeService.subscribe('internal_messages', (event) => {
      if (event.eventType === 'INSERT') {
        const newRecord = event.new as any
...
export function useActivityLogger() {
  ...
  const logActivity = async (activity: Omit<ActivityItem, 'id' | 'created_at' | 'user_id' | 'user_name'>) => {
    ...
    realtimeService.broadcast('activities', 'new_activity', fullActivity)
  }
