...
class PerformanceMonitoringService {
  ...
  private handlePerformanceEntry(entry: PerformanceEntry) {
    ...
    let metric: Partial<PerformanceMetrics> = {
      page: currentPage,
      timestamp,
    ...
    if ('connection' in navigator) {
      metric.connection = (navigator as any).connection?.effectiveType
    }
   ...
   * Custom metrik ekle
   */
  recordCustomMetric(metricName: string, value: number, metadata?: Record<string, any>) {
    const metric: PerformanceMetrics = {
      page: window.location.pathname,
      timestamp: Date.now(),
      userAgent: navigator.userAgent,
  ...
  return apiCall()
    .then((result) => {
      ...
      if (response && typeof response === 'object' && 'status' in response) {
        PerformanceService.recordAPIPerformance(endpoint, method, startTime, response)
      }
      return result
    })
    .catch((error) => {
