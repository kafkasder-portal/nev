...
export const messagesApi = {
  ...
  async editMessage(_messageId: string, _content: string): Promise<InternalMessage> {
    try {
      // Mock update
      const messages = generateMockMessages('conv_1')
      const existingMessage = messages.find(m => m.id === _messageId)
  ...
  subscribeToConversation(_conversationId: string, _callback: (message: InternalMessage) => void) {
    ...
    // Mock subscription
    console.log(`Subscribed to conversation ${_conversationId}`)
    return {
      unsubscribe: () => console.log(`Unsubscribed from conversation ${_conversationId}`)
    }
  },

  subscribeToNotifications(_userId: string, _callback: (notification: MessageNotification) => void) {
    // In real implementation, set up Supabase real-time subscription
    // return supabase
    ...
    // Mock subscription
    console.log(`Subscribed to notifications for user ${_userId}`)
    return {
      unsubscribe: () => console.log(`Unsubscribed from notifications for user ${_userId}`)
    }
  },
