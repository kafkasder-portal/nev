...
export class WorkflowEngine {
  ...
  async executeCommand(
    ...
    try {
      ...
      // 1. NLP ile komut analizi
      const nlpResult = await nlpProcessor.processText(command)
      ...
      if (this.isSimpleCommand(processedCommand)) {
        ...
        const workflow = await this.createWorkflow(processedCommand, nlpResult)
